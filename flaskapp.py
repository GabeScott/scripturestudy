...
def create():
    ...
    sql="""
    INSERT INTO note (id, body, tags, owner, created_by) VALUES (%s, %s, %s, %s, %s);
    """

    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(sql, (id, body, tags, owner, creator))
    conn.commit()
...
def getTags():
    ...
    sql="""
    SELECT tags FROM note WHERE (created_by = %s or owner = 'public');
    """
...
def search_by_id():
    ...
    SELECT body, tags
    FROM note
    WHERE id = %s and (owner = 'public' or created_by = %s);
    """
...
def search_by_tag():
    ...
    SELECT id, body, tags
    FROM note
    WHERE position(LOWER(%s) in LOWER(tags))>0 and (owner = 'public' or created_by = %s);
    """
...
def search_by_text():
    ...
    SELECT id, body, tags
    FROM note
    WHERE position(LOWER(%s) in LOWER(body))>0 and (owner = 'public' or created_by = %s);
    """
